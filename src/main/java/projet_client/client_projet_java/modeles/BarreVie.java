...
public class BarreVie extends Objet{
    ...
    public void render(GraphicsContext graphics) {
        ...
        if(this.position.equals("droite")){
            graphics.fillRoundRect(this.x + (SCALE * (this.vieMax - this.vie)) - (this.vieMax * SCALE) - (SCALE), this.y + (1.5 * SCALE), this.vie * SCALE, 12 * SCALE, 20, 20);
        } else if(this.position.equals("gauche")){
            graphics.fillRoundRect(this.x + 100 + (SCALE), this.y + (1.5 * SCALE), this.vie * SCALE, 12 * SCALE, 20, 20);
        }
    }
}
